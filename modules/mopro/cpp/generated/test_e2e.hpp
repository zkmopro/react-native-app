...
class NativeTestE2e : public jsi::HostObject {
private:
...
protected:
  ...
  jsi::Value cpp_uniffi_internal_fn_func_ffi__arraybuffer_to_string(
      jsi::Runtime &rt, const jsi::Value &thisVal, const jsi::Value *args,
      size_t count);
  jsi::Value cpp_uniffi_test_e2e_fn_func_greet(jsi::Runtime &rt,
                                               const jsi::Value &thisVal,
                                               const jsi::Value *args,
                                               size_t count);
  jsi::Value cpp_uniffi_test_e2e_fn_func_hello_uniffi(jsi::Runtime &rt,
                                                      const jsi::Value &thisVal,
                                                      const jsi::Value *args,
                                                      size_t count);
  jsi::Value cpp_uniffi_test_e2e_checksum_func_greet(jsi::Runtime &rt,
                                                     const jsi::Value &thisVal,
                                                     const jsi::Value *args,
                                                     size_t count);
  jsi::Value cpp_uniffi_test_e2e_checksum_func_hello_uniffi(
      jsi::Runtime &rt, const jsi::Value &thisVal, const jsi::Value *args,
      size_t count);
  jsi::Value cpp_ffi_test_e2e_uniffi_contract_version(jsi::Runtime &rt,
                                                      const jsi::Value &thisVal,
                                                      const jsi::Value *args,
                                                      size_t count);

public:
  NativeTestE2e(jsi::Runtime &rt,
                std::shared_ptr<uniffi_runtime::UniffiCallInvoker> callInvoker);
  virtual ~NativeTestE2e();

  /**
   * The entry point into the crate.
   *
   * React Native must call `NativeTestE2e.registerModule(rt, callInvoker)`
   ...
   * be the case.
   *
   * Clients should call `NativeTestE2e.unregisterModule(rt)` after final use
   * where possible.
   */
